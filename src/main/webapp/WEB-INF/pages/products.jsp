                    ...
                    <select name="searchCategory" class="form-control">
                        ...
                        <c:forEach items="${categories}" var="category">
                            <option value="${category.id}" <c:if
                                    test="${param.searchCategory == category.id}"> Selected</c:if>>${category.name}</option>
                        </c:forEach>
                    </select>
                </div>
            </div>
                                            ...
                                            <div class="booking-item-flight-details">
                                                <div class="booking-item-arrival">
                                                    <p class="booking-item-date"
                                                       style="font-size: 14px;!important">${product.category.name}</p>
                                                </div>
                                                <div class="booking-item-departure">
                                                    <p class="booking-item-date"
                                                       style="font-size: 14px;!important">${product.name}</p>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <p><b>Sizes:</b> <c:forEach items="${product.categorySizes}"
                                                                        var="size">${size.size} </c:forEach></p>
                                        </div>
                                        <div class="col-md-2"><span class="booking-item-price"
                                                                    style="font-size: 24px;!important">$${product.price}</span>
                                        </div>
                                        <div class="col-md-2">
                                            <a class="btn btn-primary"
                                               href="/admin/products/update/${product.id}">Edit</a>&nbsp;&nbsp;&nbsp;&nbsp;
